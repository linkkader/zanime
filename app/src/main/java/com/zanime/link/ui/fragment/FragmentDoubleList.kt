...
import com.zanime.link.adapters.AdapterTopList
import com.zanime.link.database.Top
import com.zanime.link.database.TopViewModel
...
class FragmentDoubleList(var url : String,val source : String,var genre:String) : Fragment() {
    companion object{
        @JvmStatic
        fun newInstance( url : String,source : String, genre:String) =
            FragmentDoubleList(url,source,genre).apply {
                arguments = Bundle().apply {
                    putString("kader", url+source+genre)
                }
            }
    }
    var listPage = listOf<String>()
    var i = 0
    var check = 0
    var position =1
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val adapter = AdapterTopList(context)
        ...
        recycler.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrollStateChanged(recyclerView: RecyclerView, newState: Int) {
                ...
                if (!recyclerView.canScrollVertically(1)&&check==0) {
                    ...
                    else if (listPage.isNotEmpty()&&position<listPage.size){
                        jsoupResponse(listPage[position],source,genre)
                    }
                }
            }
        })
        listViewModel = ViewModelProvider(this@FragmentDoubleList).get(TopViewModel(activity!!.application)::class.java)
            ...
            try {
                ...
                if (tag=="voiranime"){
                    ...
                    if (genre=="search"){
                        ...
                        if (listPage.isEmpty()){
                            elements = doc.select("div.content-inner").select("center")
                            elements = elements[elements.size-2].select("a")
                            for (element in elements){
                                var url = "https://www.ianimes.org/"+element.toString().substringAfter("href=\"").substringBefore("\"")
                                url = url.replace("&amp;","&")
                                listPage = listPage.plus(url)
                            }
                        }
                        check = 0
                    }
